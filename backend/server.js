...
const db = require('./db'); // new module for pg Pool

const app = express();
const port = process.env.PORT || 5000; // corrected default port
...
app.use('/uploads', express.static(process.env.UPLOAD_DIR || path.join(__dirname, 'uploads', 'reports')));

// Multer storage configuration using UPLOAD_DIR
const UPLOAD_DIR = process.env.UPLOAD_DIR || path.join(__dirname, 'uploads', 'reports');

const storage = multer.diskStorage({
  destination: async (req, file, cb) => {
    const uploadDir = path.isAbsolute(UPLOAD_DIR) ? UPLOAD_DIR : path.join(__dirname, UPLOAD_DIR);
...
app.post('/api/reports/submit', authenticateToken, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const values = [
      ...
      Array.isArray(keywords) ? keywords : JSON.parse(keywords || '[]'),
      abstract,
...
app.listen(port, () => {
  ...
  console.log(`Database: ${process.env.DATABASE_URL ? 'Configured via DATABASE_URL' : 'Not configured'}`);
});
